...
export default function CartPage() {
  ...
  const { state, removeFromCart, updateCartItem, clearCart, addOrder } = appContext;
  ...
  const handleDeliverySubmit = async (formData: DeliveryFormData) => {
    ...
    try {
      ...
      // Создаем заказ (id будет перезаписан в addOrder)
      const orderData: Order = {
        id: 'temp-id', // Временный id, будет перезаписан
        items: state.cart,
      ...
      // Здесь можно добавить редирект на страницу заказов или показать уведомление
    } catch (error) {
